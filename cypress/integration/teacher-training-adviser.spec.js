...
describe(
	'Feature - Get an adviser : Tests execution date and time : ' + new Date(),
	() => {
		var returner;
		...
		beforeEach(function () {
			...
			cy.fixture('mailinglist-signup-test-data.json').then(
				(testInputData) => {
		...
		*/
			...
			cy.clickOnContinueButton();
			cy.get(
				'#teacher-training-adviser-steps-uk-callback-telephone-field'
			).type('0123454748');
			/*cy.get(".govuk-heading-l")
			.should("exist")
		...
		*/
		...
		it('It should retain the candidate details if he navigates back  ', function () {
			...
			cy.contains('Which subject is your degree?')
				.next()
				.contains('Biology');
		...
		it('It shows the error message if user clicks continiue button without entering the mandatory or correct details', function () {
			...
			cy.get('li > a')
				...
				.should(
					'have.text',
					'Select yes if you are returning to teaching'
				);
			cy.get(
			...
			cy.get(
				'#teacher-training-adviser-steps-have-a-degree-degree-options-field-error'
			).click();
			teacherTrainingAdviser.getContinueButton().click();
			teacherTrainingAdviser.getContinueButton().click();
			cy.selectWhichClassIsYourDegree('First class');
			...
			cy.get('li > a')
				...
				.should(
					'have.text',
					'You must select either primary or secondary'
				);
			cy.get(
			...
			cy.get('li > a')
				...
				.should(
					'have.text',
					'Select if you live in the UK or overseas'
				);
			cy.get(
			...
			cy.get(
				'#teacher-training-adviser-steps-uk-or-overseas-uk-or-overseas-field-error'
			).click();
			teacherTrainingAdviser.getContinueButton().click();
			teacherTrainingAdviser.getContinueButton().click();
			cy.get('#error-summary-title')
			...
			).type(this.testData.phoneNumber);
			teacherTrainingAdviser.getContinueButton().click();
			teacherTrainingAdviser.getContinueButton().click();
		...
		it('It allows user to change his details on answers page is he wishes - Email change', function () {
			...
			teacherTrainingAdviser
				.getEmailAddress()
				.type('kumarsushant@yahoomail.com');
			teacherTrainingAdviser.getContinueButton().click();
			cy.contains('No')
				.invoke('attr', 'for')
				.then(function (val) {
		...
		it('It shows error message to user if he enters invalid telephone number - UK user', function () {
			...
			cy.get('#error-summary-title')
				...
				.should('have.text', 'There is a problem');
			cy.get(
				'#teacher-training-adviser-steps-uk-telephone-telephone-error'
			)
				.should('exist')
				.should(
					'have.text',
		...
		it('It shows error message to user if he enters invalid telephone number - overseas user', function () {
			...
			cy.get('#error-summary-title')
				...
				.should('have.text', 'There is a problem');
			cy.get(
				'#teacher-training-adviser-steps-overseas-telephone-telephone-error'
			)
				.should('exist')
				.should(
					'have.text',
...
describe('Matchback feature', () => {
	...
	it('It prevents mailing list sign up if user already signed up for a teacher training adviser service', function () {
		cy.readFile('cypress/fixtures/user.txt').then((value) => {
			...
			cy.get('h1')
				...
				.should(
					'have.text',
					'You have already signed up to an adviser'
				);
		});
	});
	...
	it('It shows "You’ve signed up" ', function () {
		...
		mailingListSignUp
			.getStage()
			.select(
				this.testInputData.howCloseAreYoutoApplyingForTeacherTraining
			);
		mailingListSignUp.getNextStep().click();
		mailingListSignUp
			.getSubjectToTeach()
		...
		cy.get(
			'#mailing-list-steps-contact-accept-privacy-policy-1-field'
		).click();
