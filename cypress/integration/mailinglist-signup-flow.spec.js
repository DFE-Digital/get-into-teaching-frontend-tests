...
describe(`Feature - Mailing list sign up : Tests execution date and time : ${new Date()}`, () => {
	...
	beforeEach(function () {
		cy.fixture('mailinglist-signup-test-data.json').then((testInputData) => { this.testInputData = testInputData; });
	...
	it('It shows the error message if user clicks next button without entering the mandatory details', function () {
		...
		cy.get('.govuk-error-summary__list').children().should('exist').next().should('exist')
		.next().should('exist');
		cy.get('.govuk-list.govuk-error-summary__list > li:nth-child(1)').should('have.text', 'Enter your full email address')
		.next().should('have.text', 'Enter your first name').next().should('have.text', 'Enter your last name');
	});
	...
	it('Error message link navigates to its respective field', function () {
		...
		mailingListSignUp.getSubmitButton().click();
		cy.get('.govuk-list.govuk-error-summary__list > li:nth-child(1)').should('have.text', 'Enter your first name')
		.next().should('have.text', 'Enter your last name');
	...
	it('It shows error message if user enters wrong email verification code', function () {
		...
		mailingListSignUp.getNextStep().click();
		cy.get('#mailing-list-steps-authenticate-timed-one-time-password-error').should('exist')
	...
	it('It resends the verification code to users email', function () {
		...
		).should('exist').should('have.text', 'You’ve already signed up');
	});
